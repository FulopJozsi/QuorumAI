...
**Memória és tudás.** A hosszú távú memória Qdrantban él — hibrid szemantikus + lexikális (BM42) kereséssel, deduplikálva, és éjszakánként konszolidálva a session-előzményekből tartós tényekké. Egy FalkorDB tudásgráf felhasználónként követi az entitásokat és kapcsolataikat, így az agensek tudják, kivel beszélnek. Egy tudásbázis PDF-eket, DOCX-eket, táblázatokat és egyebeket dolgoz fel, amelyeket bármelyik agens visszakereshet. Az asszociatív memória tovább megy: a felidézett emlékből a tudásgráfon haladva olyan kapcsolódó tényeket is felszínre hoz, amikre rá se kérdeztél, így az agens magától köti össze a szálakat.
...
- **Tudásgráf** — FalkorDB (Redis-kompatibilis), felhasználó-szintű Cypher lekérdezések. Az entitások és kapcsolatok minden kör végén automatikusan kinyerődnek, vagy az agens explicit módon írhatja őket a `graph_remember_entity`/`graph_remember_relation` eszközökkel; a gráf-kontextus automatikusan bekerül a rendszerüzenetbe (`memory_graph: true` alapértelmezetten), és megjelenik a GUI Megfigyelhetőség fülén. A gráf a GUI-n nemcsak megtekinthető, hanem szerkeszthető is.
