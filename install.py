...
LANGS: Dict[str, Dict[str, str]] = {
    "en": {
        ...
        "satellite_bash_key_prompt": "must match BASH_MCP_API_KEY in the .env of the machine running the orchestrator; blank = generate",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY generated. Copy it into the .env of the machine running the orchestrator (both sides must match):",
        ...
        "omnivoice_gpu_hint": "OmniVoice: NVIDIA GPU + driver + nvidia-container-toolkit recommended. Works without a GPU too (CPU mode is set up automatically), but synthesis will be slow.",
    ...
    "hu": {
        ...
        "satellite_bash_key_prompt": "egyeznie kell az orchestratort futtató gép .env-jében lévő BASH_MCP_API_KEY-jel; üresen hagyva generál",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY legenerálva. Másold át az orchestratort futtató gép .env-jébe (a két oldalnak egyeznie kell):",
        ...
        "omnivoice_gpu_hint": "OmniVoice: NVIDIA GPU + illesztőprogram + nvidia-container-toolkit ajánlott. GPU nélkül is működik (a CPU mód automatikusan beállítódik), de a hangszintézis lassú lesz.",
    ...
    "de": {
        ...
        "omnivoice_gpu_hint": "OmniVoice: NVIDIA-GPU + Treiber + nvidia-container-toolkit empfohlen. Funktioniert auch ohne GPU (CPU-Modus wird automatisch eingerichtet), die Synthese ist dann aber langsam.",
        ...
        "satellite_bash_key_prompt": "muss mit BASH_MCP_API_KEY in der .env des Orchestrator-Rechners übereinstimmen; leer = generieren",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY generiert. In die .env des Orchestrator-Rechners kopieren (beide Seiten müssen übereinstimmen):",
    ...
    "fr": {
        ...
        "omnivoice_gpu_hint": "OmniVoice : GPU NVIDIA + pilote + nvidia-container-toolkit recommandés. Fonctionne aussi sans GPU (le mode CPU est configuré automatiquement), mais la synthèse sera lente.",
        ...
        "satellite_bash_key_prompt": "doit correspondre à BASH_MCP_API_KEY dans le .env de la machine de l'orchestrateur ; vide = générer",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY généré. Copiez-le dans le .env de la machine de l'orchestrateur (les deux côtés doivent correspondre) :",
    ...
    "es": {
        ...
        "omnivoice_gpu_hint": "OmniVoice: se recomienda GPU NVIDIA + controlador + nvidia-container-toolkit. Funciona también sin GPU (el modo CPU se configura automáticamente), pero la síntesis será lenta.",
        ...
        "satellite_bash_key_prompt": "debe coincidir con BASH_MCP_API_KEY en el .env de la máquina del orquestador; vacío = generar",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY generada. Cópiala en el .env de la máquina del orquestador (ambos lados deben coincidir):",
    ...
    "pt": {
        ...
        "satellite_bash_key_prompt": "tem de coincidir com BASH_MCP_API_KEY no .env da máquina do orquestrador; vazio = gerar",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY gerada. Copie-a para o .env da máquina do orquestrador (os dois lados têm de coincidir):",
        ...
        "omnivoice_gpu_hint": "OmniVoice: recomenda-se GPU NVIDIA + driver + nvidia-container-toolkit. Também funciona sem GPU (o modo CPU é configurado automaticamente), mas a síntese será lenta.",
    },
    "ru": {
        "lang_name": "Русский",
        ...
        "omnivoice_gpu_hint": "OmniVoice: рекомендуется GPU NVIDIA + драйвер + nvidia-container-toolkit. Работает и без GPU (режим CPU настраивается автоматически), но синтез будет медленным.",
        ...
        "satellite_bash_key_prompt": "должен совпадать с BASH_MCP_API_KEY в .env машины с оркестратором; пусто = сгенерировать",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY сгенерирован. Скопируйте его в .env машины с оркестратором (обе стороны должны совпадать):",
    ...
    "nl": {
        ...
        "omnivoice_gpu_hint": "OmniVoice: NVIDIA-GPU + driver + nvidia-container-toolkit aanbevolen. Werkt ook zonder GPU (CPU-modus wordt automatisch ingesteld), maar de synthese is dan traag.",
        ...
        "satellite_bash_key_prompt": "moet overeenkomen met BASH_MCP_API_KEY in de .env van de orchestrator-machine; leeg = genereren",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY gegenereerd. Kopieer hem naar de .env van de orchestrator-machine (beide kanten moeten gelijk zijn):",
    ...
    "pl": {
        ...
        "omnivoice_gpu_hint": "OmniVoice: zalecane GPU NVIDIA + sterownik + nvidia-container-toolkit. Działa też bez GPU (tryb CPU konfiguruje się automatycznie), ale synteza będzie wolna.",
        ...
        "satellite_bash_key_prompt": "musi zgadzać się z BASH_MCP_API_KEY w .env maszyny z orkiestratorem; puste = wygeneruj",
        "satellite_bash_key_generated": "Wygenerowano BASH_MCP_API_KEY. Skopiuj go do .env maszyny z orkiestratorem (obie strony muszą się zgadzać):",
    ...
    "uk": {
        ...
        "omnivoice_gpu_hint": "OmniVoice: рекомендовано GPU NVIDIA + драйвер + nvidia-container-toolkit. Працює й без GPU (режим CPU налаштовується автоматично), але синтез буде повільним.",
        ...
        "satellite_bash_key_prompt": "має збігатися з BASH_MCP_API_KEY у .env машини з оркестратором; порожньо = згенерувати",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY згенеровано. Скопіюйте його в .env машини з оркестратором (обидві сторони мають збігатися):",
    ...
    "sv": {
        ...
        "omnivoice_gpu_hint": "OmniVoice: NVIDIA-GPU + drivrutin + nvidia-container-toolkit rekommenderas. Fungerar även utan GPU (CPU-läge ställs in automatiskt), men syntesen blir långsam.",
        ...
        "satellite_bash_key_prompt": "måste matcha BASH_MCP_API_KEY i .env på orkestratorns maskin; tomt = generera",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY genererad. Kopiera den till .env på orkestratorns maskin (båda sidor måste matcha):",
    ...
    "it": {
        ...
        "omnivoice_gpu_hint": "OmniVoice: consigliati GPU NVIDIA + driver + nvidia-container-toolkit. Funziona anche senza GPU (la modalità CPU viene configurata automaticamente), ma la sintesi sarà lenta.",
        ...
        "satellite_bash_key_prompt": "deve coincidere con BASH_MCP_API_KEY nel .env della macchina dell'orchestratore; vuoto = genera",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY generata. Copiala nel .env della macchina dell'orchestratore (i due lati devono coincidere):",
    ...
    "ro": {
        ...
        "satellite_bash_key_prompt": "trebuie să coincidă cu BASH_MCP_API_KEY din .env-ul mașinii orchestratorului; gol = generează",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY generat. Copiați-l în .env-ul mașinii orchestratorului (ambele părți trebuie să coincidă):",
        ...
        "omnivoice_gpu_hint": "OmniVoice: se recomandă GPU NVIDIA + driver + nvidia-container-toolkit. Funcționează și fără GPU (modul CPU se configurează automat), dar sinteza va fi lentă.",
    ...
    "cs": {
        ...
        "satellite_bash_key_prompt": "musí odpovídat BASH_MCP_API_KEY v .env stroje s orchestrátorem; prázdné = vygenerovat",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY vygenerován. Zkopírujte ho do .env stroje s orchestrátorem (obě strany se musí shodovat):",
        ...
        "omnivoice_gpu_hint": "OmniVoice: doporučeno GPU NVIDIA + ovladač + nvidia-container-toolkit. Funguje i bez GPU (režim CPU se nastaví automaticky), syntéza však bude pomalá.",
    ...
    "sk": {
        ...
        "satellite_bash_key_prompt": "musí zodpovedať BASH_MCP_API_KEY v .env stroja s orchestrátorom; prázdne = vygenerovať",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY vygenerovaný. Skopírujte ho do .env stroja s orchestrátorom (obe strany sa musia zhodovať):",
        ...
        "omnivoice_gpu_hint": "OmniVoice: odporúča sa GPU NVIDIA + ovládač + nvidia-container-toolkit. Funguje aj bez GPU (režim CPU sa nastaví automaticky), syntéza však bude pomalá.",
    ...
    "bg": {
        ...
        "satellite_bash_key_prompt": "трябва да съвпада с BASH_MCP_API_KEY в .env на машината с оркестратора; празно = генериране",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY е генериран. Копирайте го в .env на машината с оркестратора (двете страни трябва да съвпадат):",
        ...
        "omnivoice_gpu_hint": "OmniVoice: препоръчва се GPU NVIDIA + драйвер + nvidia-container-toolkit. Работи и без GPU (CPU режимът се настройва автоматично), но синтезът ще е бавен.",
    ...
    "hr": {
        ...
        "satellite_bash_key_prompt": "mora odgovarati BASH_MCP_API_KEY u .env računala s orkestratorom; prazno = generiraj",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY generiran. Kopirajte ga u .env računala s orkestratorom (obje strane moraju odgovarati):",
        ...
        "omnivoice_gpu_hint": "OmniVoice: preporučuje se NVIDIA GPU + upravljački program + nvidia-container-toolkit. Radi i bez GPU-a (CPU način se postavlja automatski), ali će sinteza biti spora.",
    ...
    "sl": {
        ...
        "satellite_bash_key_prompt": "se mora ujemati z BASH_MCP_API_KEY v .env računalnika z orkestratorjem; prazno = ustvari",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY ustvarjen. Kopirajte ga v .env računalnika z orkestratorjem (obe strani se morata ujemati):",
        ...
        "omnivoice_gpu_hint": "OmniVoice: priporočljiv NVIDIA GPU + gonilnik + nvidia-container-toolkit. Deluje tudi brez GPU-ja (način CPU se nastavi samodejno), a bo sinteza počasna.",
    ...
    "el": {
        ...
        "satellite_bash_key_prompt": "πρέπει να ταιριάζει με το BASH_MCP_API_KEY στο .env του μηχανήματος του orchestrator· κενό = δημιουργία",
        "satellite_bash_key_generated": "Το BASH_MCP_API_KEY δημιουργήθηκε. Αντιγράψτε το στο .env του μηχανήματος του orchestrator (οι δύο πλευρές πρέπει να ταιριάζουν):",
        ...
        "omnivoice_gpu_hint": "OmniVoice: συνιστάται GPU NVIDIA + πρόγραμμα οδήγησης + nvidia-container-toolkit. Λειτουργεί και χωρίς GPU (η λειτουργία CPU ρυθμίζεται αυτόματα), αλλά η σύνθεση θα είναι αργή.",
    ...
    "da": {
        ...
        "satellite_bash_key_prompt": "skal matche BASH_MCP_API_KEY i .env på orkestratorens maskine; tom = generér",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY genereret. Kopiér den til .env på orkestratorens maskine (begge sider skal matche):",
        ...
        "omnivoice_gpu_hint": "OmniVoice: NVIDIA-GPU + driver + nvidia-container-toolkit anbefales. Virker også uden GPU (CPU-tilstand opsættes automatisk), men syntesen bliver langsom.",
    ...
    "fi": {
        ...
        "satellite_bash_key_prompt": "täytyy vastata orkestraattorikoneen .env-tiedoston BASH_MCP_API_KEY-arvoa; tyhjä = luo",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY luotu. Kopioi se orkestraattorikoneen .env-tiedostoon (molempien puolten on vastattava):",
        ...
        "omnivoice_gpu_hint": "OmniVoice: suositellaan NVIDIA-GPU + ajuri + nvidia-container-toolkit. Toimii myös ilman GPU:ta (CPU-tila määritetään automaattisesti), mutta synteesi on hidasta.",
    ...
    "lt": {
        ...
        "satellite_bash_key_prompt": "turi sutapti su BASH_MCP_API_KEY orkestratoriaus kompiuterio .env faile; tuščia = sugeneruoti",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY sugeneruotas. Nukopijuokite jį į orkestratoriaus kompiuterio .env (abi pusės turi sutapti):",
        ...
        "omnivoice_gpu_hint": "OmniVoice: rekomenduojama NVIDIA GPU + tvarkyklė + nvidia-container-toolkit. Veikia ir be GPU (CPU režimas nustatomas automatiškai), bet sintezė bus lėta.",
    ...
    "lv": {
        ...
        "satellite_bash_key_prompt": "jāsakrīt ar BASH_MCP_API_KEY orķestratora datora .env; tukšs = ģenerēt",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY ģenerēts. Nokopējiet to orķestratora datora .env (abām pusēm jāsakrīt):",
        ...
        "omnivoice_gpu_hint": "OmniVoice: ieteicams NVIDIA GPU + draiveris + nvidia-container-toolkit. Darbojas arī bez GPU (CPU režīms tiek iestatīts automātiski), taču sintēze būs lēna.",
    ...
    "et": {
        ...
        "satellite_bash_key_prompt": "peab ühtima orkestraatori masina .env-i BASH_MCP_API_KEY väärtusega; tühi = genereeri",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY genereeritud. Kopeeri see orkestraatori masina .env-i (mõlemad pooled peavad ühtima):",
        ...
        "omnivoice_gpu_hint": "OmniVoice: soovitatav NVIDIA GPU + draiver + nvidia-container-toolkit. Töötab ka ilma GPU-ta (CPU-režiim seadistatakse automaatselt), kuid süntees on aeglane.",
    ...
    "ga": {
        ...
        "satellite_bash_key_prompt": "caithfidh sé a bheith mar an gcéanna le BASH_MCP_API_KEY i .env mheaisín an orchestrator; folamh = gin",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY ginte. Cóipeáil é isteach i .env mheaisín an orchestrator (caithfidh an dá thaobh a bheith mar an gcéanna):",
        ...
        "omnivoice_gpu_hint": "OmniVoice: moltar GPU NVIDIA + tiománaí + nvidia-container-toolkit. Oibríonn sé gan GPU freisin (socraítear mód CPU go huathoibríoch), ach beidh an tsintéis mall.",
    ...
    "mt": {
        ...
        "satellite_bash_key_prompt": "irid jaqbel ma' BASH_MCP_API_KEY fil-.env tal-magna tal-orchestrator; vojt = iġġenera",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY ġie ġġenerat. Ikkopjah fil-.env tal-magna tal-orchestrator (iż-żewġ naħat iridu jaqblu):",
        ...
        "omnivoice_gpu_hint": "OmniVoice: rakkomandat GPU NVIDIA + driver + nvidia-container-toolkit. Jaħdem ukoll mingħajr GPU (il-modalità CPU tiġi kkonfigurata awtomatikament), imma s-sinteżi tkun bil-mod.",
    ...
    "no": {
        ...
        "satellite_bash_key_prompt": "må samsvare med BASH_MCP_API_KEY i .env på orkestratorens maskin; tom = generer",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY generert. Kopier den til .env på orkestratorens maskin (begge sider må samsvare):",
        ...
        "omnivoice_gpu_hint": "OmniVoice: NVIDIA-GPU + driver + nvidia-container-toolkit anbefales. Fungerer også uten GPU (CPU-modus settes opp automatisk), men syntesen blir treg.",
    ...
    "sr": {
        ...
        "satellite_bash_key_prompt": "мора да се поклапа са BASH_MCP_API_KEY у .env рачунара са оркестратором; празно = генериши",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY је генерисан. Копирајте га у .env рачунара са оркестратором (обе стране морају да се поклапају):",
        ...
        "omnivoice_gpu_hint": "OmniVoice: препоручује се NVIDIA GPU + драјвер + nvidia-container-toolkit. Ради и без GPU-а (CPU режим се подешава аутоматски), али ће синтеза бити спора.",
    ...
    "tr": {
        ...
        "satellite_bash_key_prompt": "orkestratörün çalıştığı makinenin .env dosyasındaki BASH_MCP_API_KEY ile aynı olmalı; boş = oluştur",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY oluşturuldu. Orkestratörün çalıştığı makinenin .env dosyasına kopyalayın (iki taraf aynı olmalı):",
        ...
        "omnivoice_gpu_hint": "OmniVoice: NVIDIA GPU + sürücü + nvidia-container-toolkit önerilir. GPU olmadan da çalışır (CPU modu otomatik ayarlanır), ancak sentez yavaş olur.",
    ...
    "ja": {
        ...
        "omnivoice_gpu_hint": "OmniVoice：NVIDIA GPU＋ドライバー＋nvidia-container-toolkit を推奨。GPU なしでも動作します（CPU モードが自動設定されます）が、音声合成は遅くなります。",
        ...
        "satellite_bash_key_prompt": "オーケストレーターが動くマシンの .env にある BASH_MCP_API_KEY と一致させてください。空欄 = 生成",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY を生成しました。オーケストレーターが動くマシンの .env にコピーしてください（両側で一致が必要）：",
    ...
    "zh": {
        ...
        "omnivoice_gpu_hint": "OmniVoice：建议使用 NVIDIA GPU + 驱动 + nvidia-container-toolkit。没有 GPU 也能运行（CPU 模式会自动配置），但语音合成会很慢。",
        ...
        "satellite_bash_key_prompt": "必须与运行编排器的机器 .env 中的 BASH_MCP_API_KEY 一致；留空 = 生成",
        "satellite_bash_key_generated": "已生成 BASH_MCP_API_KEY。请将其复制到运行编排器的机器的 .env 中（两端必须一致）：",
    ...
    "ko": {
        ...
        "omnivoice_gpu_hint": "OmniVoice: NVIDIA GPU + 드라이버 + nvidia-container-toolkit 권장. GPU 없이도 동작하지만(CPU 모드 자동 설정) 음성 합성이 느립니다.",
        ...
        "satellite_bash_key_prompt": "오케스트레이터가 실행되는 머신의 .env에 있는 BASH_MCP_API_KEY와 같아야 합니다. 비워 두면 생성",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY가 생성되었습니다. 오케스트레이터가 실행되는 머신의 .env에 복사하세요(양쪽이 같아야 함):",
    ...
    "sw": {
        ...
        "omnivoice_gpu_hint": "OmniVoice: inapendekezwa GPU ya NVIDIA + dereva + nvidia-container-toolkit. Inafanya kazi pia bila GPU (hali ya CPU husanidiwa kiotomatiki), lakini usanisi utakuwa wa polepole.",
        ...
        "satellite_bash_key_prompt": "lazima ilingane na BASH_MCP_API_KEY kwenye .env ya mashine inayoendesha orchestrator; tupu = tengeneza",
        "satellite_bash_key_generated": "BASH_MCP_API_KEY imetengenezwa. Inakili kwenye .env ya mashine inayoendesha orchestrator (pande zote mbili lazima zilingane):",
...
MODULES = [
    {
        ...
        "env_vars": [
            ...
            ("BASH_MCP_API_KEY", False, "", "Key between the orchestrator and the native host bash MCP (bash-mcp-native); empty = generated"),
    ...
    },
    {
        ...
        "required": True,
        "satellite": True,
        # A compose SERVICE neve "memory" (a konténer quorum-memory, az image
        # qdrant/qdrant) — a services lista compose up/stop parancsokat hajt,
        # ide a service-név kell! ("qdrant" névvel a modul-módosítási ág
        # `no such service` hibát adott.)
        "services": ["memory"],
    ...
    },
    {
        ...
        "required": True,
        "satellite": False,
        "services": ["hu-tools"],  # a compose service neve hu-tools (nem local-basic-tools)
    ...
    },
    {
        "id": "stt-tts",
        "profile": "stt-tts",
        "label": "STT/TTS backend (Wyoming Whisper + Piper) - no GPU needed",
        "required": False,
        "satellite": True,
        "default_selected": True,
        # Wyoming Whisper/Piper are shared with the `mic` layer (both on the
        # `mic` + `stt-tts` profiles). The orchestrator voice tools reach
        # Whisper/Piper over the Wyoming protocol on quorum-net.
        # OmniVoice was split into its own component (2026-09-02): it is the
        # ONLY service needing CUDA/VRAM — without it the whole voice stack
        # installs and runs on machines with no NVIDIA docker setup.
        "services": ["whisper", "piper"],
        "ports": [],
        "env_vars": [
    ...
    },
    {
        "id": "omnivoice",
        "profile": "omnivoice",
        "label": "OmniVoice neural TTS (voice cloning, HU narration) - NVIDIA GPU recommended, slow on CPU",
        "required": False,
        "satellite": True,
        "default_selected": False,
        # Split from stt-tts (2026-09-02): the only GPU/CUDA-dependent service.
        # Used by the video agent (HyperFrames Hungarian narration) and the
        # orchestrator /tts proxy. On GPU-less hosts the installer strips the
        # nvidia deploy block (CPU mode, works but slow).
        "services": ["omnivoice"],
        "ports": [("OmniVoice", "OMNIVOICE_PORT", 5002)],
        "env_vars": [],
        "data_dirs": ["data/omnivoice"],
    },
    {
        "id": "telegram",
        "profile": "telegram",
    ...
    },
    {
        ...
        "required": False,
        "satellite": True,
        "services": ["home-assistant-mcp"],  # a compose service teljes neve
    ...
    },
    {
        ...
        "env_vars": [
            ...
            ("BASH_MCP_API_KEY", False, "", "Key for the native host bash MCP (X-Bash-MCP-Key); empty = generated"),
        ],
...
def build_env(
    ...
    _simple_autogen = {"ORCHESTRATOR_API_KEY", "CONVERSATION_API_KEY", "BASH_MCP_API_KEY"}
    _vapid_needed = False
    for _m in modules:
    ...
    for m in modules:
        for key, required, default, hint in m.get("env_vars", []):
            ...
            if key in _simple_autogen:
                # a blank answer ("" in env_vars) must not suppress the generated secret
                val = env_vars.get(key) or existing_env.get(key) or _auto_secrets.get(key, default)
            if val:
    ...
    # Phase 86: the mandatory orchestrator <-> native host bash MCP key is ALWAYS
    # written, even without a bash module selected, so a later native install
    # finds it. An existing value is kept: both sides read it from this .env and
    # must match. (Satellite on another machine: copy it from the main .env.)
    if "BASH_MCP_API_KEY" not in seen_keys:
        _bk = env_vars.get("BASH_MCP_API_KEY") or existing_env.get("BASH_MCP_API_KEY") or _gen_token()
        lines.append("")
        lines.append("# Native host bash MCP key (X-Bash-MCP-Key), generated")
        lines.append(f"BASH_MCP_API_KEY={_bk}")
...
def modify_mode(s: Dict[str, str], install_dir: Path) -> None:
    ...
    if any(m["id"] == "omnivoice" for m in added):
        print(t(s, "omnivoice_gpu_hint"))
...
def fresh_install(s: Dict[str, str], install_dir: Path, satellite: bool = False) -> None:
    ...
    if satellite:
        ...
        # Phase 86: the native host bash MCP on this satellite must share the key
        # with the orchestrator's .env; blank = generate here and show it to copy.
        satellite_bash_key = os.environ.get("BASH_MCP_API_KEY") or ask_password(
            f"  BASH_MCP_API_KEY ({t(s, 'satellite_bash_key_prompt')})"
        )
        if not satellite_bash_key:
            satellite_bash_key = _gen_token()
            print(f"  [i] {t(s, 'satellite_bash_key_generated')}\n      BASH_MCP_API_KEY={satellite_bash_key}")
    ...
    if any(m["id"] == "omnivoice" for m in modules):
        print(t(s, "omnivoice_gpu_hint"))
    ...
    if satellite:
        env_vars["BASH_MCP_API_KEY"] = satellite_bash_key
    if any(m["id"] == "mic" for m in modules):
