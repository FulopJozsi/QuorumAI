...
MODULES = [
    ...
    },
    {
        ...
        "ports": [],
        "env_vars": [
            ("WAKE_WORD", False, "Ok Szif", "Wake word phrase"),
            ("WAKE_WORD_FILENAME", False, "ok_sif", "Wake word model filename (without .onnx extension)"),
            ...
            ("MIC_STREAM_TIMEOUT", False, "120", "Max seconds to wait for a streaming LLM response"),
    ...
    },
    {
        ...
        "env_vars": [
            # n8n MCP (@leonardsellem/n8n-mcp-server) runs as an npx server inside
            # the manager; the manager passes these env vars to the spawned child.
            ("N8N_API_URL", False, "", "Full n8n API URL incl. /api/v1, e.g. https://n8n.example.com/api/v1 (mcp-manager npx server)"),
            ("N8N_API_KEY", False, "", "n8n API key (Settings → API). Passed by mcp-manager to the n8n MCP"),
            ("N8N_WEBHOOK_USERNAME", False, "", "Optional n8n webhook Basic-auth username"),
            ("N8N_WEBHOOK_PASSWORD", False, "", "Optional n8n webhook Basic-auth password"),
        ],
        "data_dirs": [],
    },
    {
...
services:
  ...
  mic:
    build: .
    image: fulopjozsef86/quorum-mic:0.3.0
    container_name: quorum-mic
    env_file:
      - ../../.env
    environment:
...
def run_menu(title: str, options: List[str]) -> int:
...
def select_industry_pack(s: Dict[str, str], existing_pack: str = "") -> Optional[str]:
    """Ask user which industry pack to install. Returns pack id or '' for none."""
    ...
    if env_pack is not None:
        if env_pack in ("", "none"):
            return ""
        if any(p["id"] == env_pack for p in packs):
            return env_pack
    ...
    if _FANCY_MENU:
        opts = [
            ...
            + (f" [{t(s, 'pack_installed')}]" if p["id"] == existing_pack else "")
            for p in packs
        ]
        opts.append(t(s, "pack_none"))
        idx = run_menu(t(s, "choose"), opts)
        return "" if idx >= len(packs) else packs[idx]["id"]
    for i, p in enumerate(packs, 1):
        label = p.get(f"label_{lang_code}", p.get("label_en", p["id"]))
        desc  = p.get(f"desc_{lang_code}",  p.get("desc_en",  ""))
        marker = f" [{t(s, 'pack_installed')}]" if p["id"] == existing_pack else ""
...
def modify_mode(s: Dict[str, str], install_dir: Path) -> None:
    ...
    pack_id = select_industry_pack(s)
    ...
    install_industry_pack(pack_id, install_dir, s)
